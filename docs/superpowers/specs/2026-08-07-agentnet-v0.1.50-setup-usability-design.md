...
The setup attempt journal records phase, attempt ID, package version, request digest, transition path, elapsed monotonic time, blocker code, and terminal result. It contains no secrets or protected content.
...
The implementation computes a contiguous migration path through the existing forward-only edges and executes each internal edge under the same setup lock and journal. Every edge validates its exact source schema/unit profile and durably records its migration result before the next edge. The public setup marker advances to v0.1.50 only after the complete path and exact v0.1.50 package/unit apply succeed. A crash resumes at the first uncommitted journal edge. Unknown, ambiguous, missing, downgraded, or noncontiguous state fails closed before service restart.
