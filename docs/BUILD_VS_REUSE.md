...
|---|---|---|
| Node.js / npm | Laptop Node `26.5.0`, npm `12.0.1`; Hub Node `22.23.2` LTS, npm `11.18.0` | Useful for MCP binding and cross-runtime contract tests; not the selected core/A2A runtime. Package floor remains Node `>=22.19.0`, release/publish stays pinned to Node `24.18.0` LTS plus npm `12.0.1`, and compatibility CI covers exact Node `22.19.0`, `24.18.0`, and `26.5.0`. EOL Node 23/25 are unsupported. Node provides `node:test`, `node:sqlite`, WebCrypto/Ed25519, and native TypeScript type stripping. |
