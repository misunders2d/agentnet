...
| Human authentication and independent approval | Existing OIDC authorization-code/PKCE/JWT profile over maintained Python TLS/crypto primitives; maintained WebAuthn and approval-service libraries still require an exact pin | **OIDC profile and direct validated-address transport accepted at local H tier; real-provider adoption and WebAuthn activation deferred; WebAuthn reuse required** | Opaque domain principal mapping, exact transaction, independent boundary, recovery, approval consumption, endpoint-origin policy, and connection-address pins remain corporate | The former resolve-then-hostname-connect gap is closed locally: the socket uses the validated numeric snapshot while TLS/SNI/Host retain the configured hostname, with proxies and redirects disabled. Real IdP/TLS L/E evidence, PD-001/002/004, and a qualifying independent approval topology remain unresolved. No custom WebAuthn cryptography or same-machine approval shortcut is allowed. |
...
| Local tool binding | MCP spec `2025-11-25`; project-pinned Python SDK `1.28.1`; cached TypeScript SDK `1.29.0` only as a comparison candidate | **Accept pinned Python mechanism for the optional local adapter; production gate deferred** | Caller identity comes from authenticated local session/capability, never MCP arguments; no corporate/A2A bearer pass-through; Pi remains direct IPC | Installation is reproducible, but Gate 5 must still prove identical explicit tool round trips and Pi-without-MCP on real harnesses. MCP as the network is rejected. |
