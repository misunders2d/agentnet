...
4. The server validates issuer, audience, subject, workforce/contractor status, assurance, nonce, expiry, and proof of possession. Authorization-response parsing rejects every duplicate decoded parameter name and mixed or incomplete success/error shape, while ignoring only unique unrecognized OAuth response extensions. A provider error can terminally consume only the exact state-bound pending transaction and never reaches token exchange.
...
6. The human approves exact transaction after fresh phishing-resistant authentication but transfers no completion value. Exact waiting process retains private Core continuation/begin state. Core derives a transaction-specific OIDC Approval possession secret or generates and encrypts a distinct high-entropy bootstrap secret, sends only that secret's SHA-256 hash when creating Approval request, and later retrieves through signed broker using exact purpose-separated secret. Approval enforces expiry, cumulative mismatch attempts, replay custody, and retrieval-digest conflict checks; browser receives no secret, code, receipt, or capability. Approval service binds random challenge, canonical transaction hash, beneficiary, approver, domain, and expiry; verifies RP ID, origin, user verification, and credential status; consumes challenge once; and signs immutable approval receipt. This proves ceremony and transaction binding, not human comprehension or a trusted display on a compromised enrolling host. A notification click, agent message, auto-readable bot response, or browser session alone is insufficient.
...
| **PD-002 Enrollment/recovery** | Owner-controlled WebAuthn UV plus exact-transaction, purpose-separated possession-bound automatic Approval delivery; the waiting process retains the secret and the browser/human receives no code, receipt, continuation, or private capability; legacy 128-bit claim-code delivery is compatibility-only; confirmation is independent of the enrolling harness; default server colocation uses distinct OS identities and reports `independent_boundary_proven=false` | Separately administered approval hosting is the optional high-assurance tier; extra people, hosts, and relay channels are not ordinary-onboarding prerequisites | Recorded for the ordinary self-hosted profile on 2026-07-19 and updated for automatic delivery on 2026-07-28; high-assurance administration/recovery evidence remains open |
...
| **ID-002** Independently authenticated human confirmation | Design covered; external evidence gate | Exact transcript, WebAuthn UV outside the enrolling harness, and exact purpose-separated possession-bound broker delivery retained by the waiting process (§7.2). Browser/human receives no code or capability; legacy active claim-code delivery is compatibility-only. Shared-host default must preserve distinct OS identity/storage/credentials and report `independent_boundary_proven=false`; separate administration remains optional high-assurance evidence. |
