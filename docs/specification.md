...
16. **Human consent is independent.** Peer or model content may request approval but can never satisfy it.
...
Logical trust roles stay separate even when some modules share a deployment. Scanner, effect worker, public gateway, federation gateway, authority keys, approval service, and independent audit witness require distinct credentials and boundaries.
...
5. A pre-bound independent approval service on a **separately controlled device, OS principal, or tested hardware/security boundary inaccessible to every enrolling/enrolled harness** shows the exact typed transaction: domain, human, beneficiary harness/device, key thumbprint, requested class, nonce, and expiry. Same-machine Slack/Telegram does not qualify when harnesses share the desktop user, files, process authority, notifications, bot token, or app automation. Slack/Telegram qualifies only on an independently controlled device/session that the harness cannot read or automate.
6. The human actively transfers a high-entropy one-time code within a maximum ten-minute enrollment lifetime and approves that exact transaction after fresh phishing-resistant authentication. The approval service server binds random challenge, canonical transaction hash, beneficiary, approver, domain, and expiry; verifies RP ID, origin, user verification, and credential status; consumes the challenge once; and signs an immutable approval receipt. This proves ceremony and transaction binding, not human comprehension or a trusted display on a compromised enrolling host. Email, a notification click, an agent message, or an auto-readable bot response is insufficient.
...
Any limited owner laboratory deployment uses the same independence rule. If a noncompliant enrollment experiment is needed for synthetic lab data, it is labeled noncompliant with ID-002, cannot access protected data, contractors, authority expansion, or production A2A, and its binding can never be promoted into production.
...
| **PD-002 Enrollment/recovery** | Fresh phishing-resistant primary auth plus active one-time code on a pre-bound separately controlled device/OS/hardware boundary inaccessible to the harness, bound to exact transaction | Same-machine Slack/Telegram fails unless tested isolation prevents reading/automation; managed second device/admin proof is stronger but costlier | Qualifying boundaries/channels, recovery owners, ten-minute-or-shorter expiry, entropy, throttling |
...
| **ID-002** Independent human confirmation | Design covered; external evidence gate | Exact transcript plus active code on a separately controlled device/OS/hardware boundary inaccessible to harnesses (§7.2). Same-machine channel and automation negatives remain. |
