...
- [ ] **ORG-002 — HARD — Multiple subordinates and downward assignment.** An agent may administer one or more subordinate agents. When a verified active administration relationship explicitly includes `may_assign` and an assignment is within its preapproved scope, the subordinate agent automatically accepts the task into its durable task mailbox without requiring a new human confirmation.
  - Acceptance: Automatic acceptance proves only that the task is durably queued. An omitted deadline is replaced before commit by an exact whole-second server-derived deadline capped by the complete assignment-scope duration and strictly before relationship expiry. Generic mailbox, conversation, and supervisor reads disclose only immutable metadata, digests, and a custody reference for task/task-linked-control events, including legacy records without the marker. This implementation has no protected TaskGrant payload-release route, so execution remains unavailable rather than inheriting the administrator's permissions, data access, semantic-processing, tool, or effect authority. Ordinary non-task messages retain their normal authorized payload behavior.
...
- [ ] **ORG-006 — OPEN — Relationship lifecycle.** Define who may create, approve, modify, expire, and remove management relationships.
  ...
  - Implemented clean-start boundary: the first released storage schema is version 1 and contains the bilateral governance, consent, exception, lineage, activation-intent, and conflict relations directly. It contains no unilateral relationship table, quarantine conversion, legacy alias, or prototype-version upgrade. Startup fails closed on a missing, older, newer, or altered schema/checksum. A pre-release prototype database is not silently migrated or treated as consent; initialize a new network and obtain fresh exact consent. Rollback is restore to an absent target from a separately signed, verified same-schema backup, never metadata downgrade or authority-row transplantation.
...
| ORG-* | Bilateral lifecycle mechanics implemented; ORG-005 conflict adjudication missing; ORG-006 owner policy pending | Concept §8.2, 20.4 | Local lifecycle/migration tests; no arrival-order-independent multi-administrator conflict adjudication; no owner or production gate evidence | Proposals have zero authority; active relations grant explicit custody meta-actions, never implicit data authority |
