...
Signed AgentNet HTTP clients construct a platform-default TLS context rather
than HTTPX's bundled `certifi` default. On the Linux server profile this honors
operator-installed system roots while the hardened units continue to remove
caller-controlled CA environment overrides.

Corrective `0.1.48` resolves completed-C0 terminal credentials through the
canonical identity relationship: the credential supplies harness and epoch,
and its bound harness supplies domain and principal. The query returns only an
exact four-part match; absence or mismatch remains fail-closed. No credential
or identity table changes. The candidate adds only the exact forward-only
`0.1.47→0.1.48` five-unit marker edge for a server already carrying the
unpublished `0.1.47` marker. It preserves schema v7 and all identity,
...
That same v0.1.50→v0.1.51 boundary recognizes the exact retained Approval
hotfix that raised the generic request TTL to one hour. Setup records the
Approval configuration in upgrade journal v3 before mutation, restores the
ordinary request ceiling to 600 seconds, and preserves the distinct
...
source bytes, and every non-matching configuration fails closed.
