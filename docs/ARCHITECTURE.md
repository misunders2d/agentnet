...
`agentnet server-agent setup` is the product-owned host composition boundary for the default self-hosted Linux profile. It accepts one strict non-secret request whose sensitive values remain owner-only file references and exposes read-only `planned`, privileged `configured_not_started`/`waiting_owner_oidc_or_passkey`, and post-activation `operational` states. It is a fixed state machine, not a provider API or deployment DSL.

The wrapper owns only locked Core/Approval identities, private AgentNet roots, Approval provisioning, Core/PostgreSQL bootstrap, scanner public trust, fixed loopback ports, two hardened systemd units, bounded service start/restart, and redacted evidence. Existing operator-owned PostgreSQL, DNS, TLS/reverse-proxy routes, certificates, firewall, and secret injection remain explicit prerequisites and are never mutated. Start verifies loopback and exact public HTTPS health; post-activation start also requires public Core readiness.

Every managed file is create-or-exact-match. Differing state blocks without overwrite. Named stages and a nonblocking lock make exact reruns resume after interruption. Apply requires the exact digest returned by the frozen read-only plan; the npm launcher reproduces the request/reference digest before creating privileged runtime state, and Python rechecks the same digest before host mutation. The root-only `/var/lib/agentnet-setup/setup.json` marker binds that request, managed units, and non-secret Approval/Core configuration digests while excluding only the enrolled harness/credential labels changed by offline activation; it never claims identity, authority, HA, or production certification. OIDC/WebAuthn, guided key-possession enrollment, and offline activation remain explicit ceremonies. Remote Managers may provide immutable package guidance and inspect sanitized evidence only; target coding agents own host execution.
