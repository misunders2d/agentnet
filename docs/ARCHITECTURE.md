...
evidence tables. Migration 5 adds recoverable OIDC-begin idempotency, exact finite
current-credential renewal request custody, and one-time replay custody for the
setup-bound expired ordinary-server credential transition. Fresh SQLite and
PostgreSQL stores create v5; the tested N/N-1 Core path accepts only an exact
