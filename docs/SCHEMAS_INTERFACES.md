...
`agentnet server-agent setup` consumes strict `agentnet.server-setup.request.v1` JSON. Unknown fields and noncanonical origins, audiences, DSNs, paths, environment references, OIDC callbacks, approvers, or scanner trust fail before host mutation. Request stores no secret value: it binds absolute owner-only file references for Core/Approval environments, provider metadata, approver policy, and scanner public trust.

Every invocation emits `agentnet.server-setup.evidence.v1` with one typed status, aggregate request digest, fixed profile, package version, managed units, loopback ports, HTTPS topology, ordered step results, next action, and explicit `identity_enrolled`, `authority_granted`, and `production_durability_proven` claims. Digest binds exact request-file bytes, canonical absolute references, non-secret JSON fingerprints, and environment variable-name sets; environment values are reference-only and are never hashed or emitted. The npm launcher independently reproduces this digest before creating privileged runtime state. Blocked output contains a bounded blocker/message and all three claims false. Environment values, credentials, executable paths, signer private material, approval receipts, claim codes, and identity profiles are never output.

Plan is read-only. Apply requires root, the exact digest returned by the frozen plan, and a nonblocking host lock. AgentNet, Node.js, and `uv` come only from the current system-wide installed package/command context, never request JSON. Target coding agent must establish root-owned non-writable service-readable lineage before invoking package code as root because untrusted code cannot self-validate safely; launcher and Python package-tree checks are defense in depth. Managed files use create-or-exact-match atomic replacement; conflicting state is not overwritten. The root-only `/var/lib/agentnet-setup/setup.json` marker binds request, units, and non-secret Approval/Core config digests while excluding only offline-activation enrollment labels. Existing Approval policy must match exact requested owner OIDC and complete approver set—extra trust anchors block. Start is valid only with apply and affects only daemon reload, Approval enable/start, Core enable/restart, and exact active-state/local/public probes. Core health returns `agentnet.core.health.v1`; Approval health returns `agentnet.approval.health.v1`. Setup validates exact service role, version, origin/domain/profile/runtime identity, and rejects generic HTTP 200 responses. `operational` requires exact enrolled binding and public Core readiness; pre-enrollment start reports `waiting_owner_oidc_or_passkey`.
