...
and `expired_server_credential_replacements` for one setup-bound, replay-safe
`0.1.31 -> 0.1.32` ordinary-server binding transition per harness.
...
recovery, finite current-credential renewal custody, and one-time expired
ordinary-server replacement custody. Fresh SQLite and
