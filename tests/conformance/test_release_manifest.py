...
def _copy_release_inputs(tmp_path: Path) -> Path:
    ...
    for relative_path in (
        ...
        "evidence/local/2026-07-23-v0.1.23/manifest.json",
    ...
    shutil.copytree(
        ROOT / "evidence/local/2026-07-23-v0.1.23/artifacts",
        root / "evidence/local/2026-07-23-v0.1.23/artifacts",
    )
    return root
...
def _refresh_artifact_hash(root: Path, artifact_path: Path) -> None:
    evidence_path = root / "evidence/local/2026-07-23-v0.1.23/manifest.json"
...
def test_artifact_self_hash_cannot_replace_archive_content_validation(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    wheel = root / "evidence/local/2026-07-23-v0.1.23/artifacts/agentnet-0.1.23-py3-none-any.whl"
...
def test_sdist_unsafe_or_extra_member_is_rejected(tmp_path: Path, mutation: str) -> None:
    root = _copy_release_inputs(tmp_path)
    sdist = root / "evidence/local/2026-07-23-v0.1.23/artifacts/agentnet-0.1.23.tar.gz"
    ...
    def mutate(entries: list[tuple[tarfile.TarInfo, bytes]]) -> None:
        ...
        member = tarfile.TarInfo(f"agentnet-0.1.23/{suffix}")
        member.size = 2
        member.mode = 0o644
