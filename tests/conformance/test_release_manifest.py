...
def _copy_release_inputs(tmp_path: Path) -> Path:
    ...
    for relative_path in (
        ...
        "evidence/local/2026-08-07-v0.1.47/manifest.json",
    ...
    shutil.copytree(
        ROOT / "evidence/local/2026-08-07-v0.1.47/artifacts",
        root / "evidence/local/2026-08-07-v0.1.47/artifacts",
    )
    return root
...
def _refresh_artifact_hash(root: Path, artifact_path: Path) -> None:
    evidence_path = root / "evidence/local/2026-08-07-v0.1.47/manifest.json"
...
def test_candidate_package_evidence_cannot_remain_pending(tmp_path: Path) -> None:
    ...
    assert "0.1.47 package evidence must record completed PASS verification" in failures
    assert "0.1.47 package evidence cannot retain pending command results" in failures
...
def test_candidate_evidence_must_cover_v0145_release_blocker_surface(tmp_path: Path) -> None:
    ...
    assert "0.1.47 focused release-blocker evidence is incomplete" in failures
...
def test_sdist_contract_does_not_require_installed_root_ignore_files(tmp_path: Path) -> None:
    ...
    assert expected[".gitignore"].endswith(b".pi/\n")


def test_release_verifier_requires_candidate_artifact_ignore_policy(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    (root / ".git").mkdir()
    (root / "evidence/local/2026-08-07-v0.1.47/artifacts/.gitignore").unlink(
        missing_ok=True
    )
...
def test_deployment_ledger_and_human_manifest_drift_fail_closed(tmp_path: Path) -> None:
    ...
    human.write_text(
        ...
        .replace(
            "SHA-256 `728d53fd307550ae8779d0715fa4a6d55f0c26d03b753756bb884b3f3b9baa24`",
            "SHA-256 `" + "0" * 64 + "`",
            1,
        ),
        encoding="utf-8",
    ...
    requirements_path.write_text(
        ...
        .replace("Snapshot: 2026-08-07.", "Snapshot: 2026-07-25.", 1)
        .replace(
    ...
    gate_path.write_text(
        gate_path.read_text(encoding="utf-8").replace(
            "Current ledger update: 2026-08-07.",
...
def test_artifact_self_hash_cannot_replace_archive_content_validation(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    wheel = root / "evidence/local/2026-08-07-v0.1.47/artifacts/agentnet-0.1.47-py3-none-any.whl"
...
def test_sdist_unsafe_or_extra_member_is_rejected(tmp_path: Path, mutation: str) -> None:
    root = _copy_release_inputs(tmp_path)
    sdist = root / "evidence/local/2026-08-07-v0.1.47/artifacts/agentnet-0.1.47.tar.gz"
    ...
    def mutate(entries: list[tuple[tarfile.TarInfo, bytes]]) -> None:
        ...
        member = tarfile.TarInfo(f"agentnet-0.1.47/{suffix}")
        member.size = 2
        member.mode = 0o644
