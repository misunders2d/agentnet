...
def _copy_release_inputs(tmp_path: Path) -> Path:
    ...
    for relative_path in (
        ...
        "evidence/local/2026-07-27-v0.1.29/manifest.json",
    ...
    shutil.copytree(
        ROOT / "evidence/local/2026-07-27-v0.1.29/artifacts",
        root / "evidence/local/2026-07-27-v0.1.29/artifacts",
    )
    return root
...
def _refresh_artifact_hash(root: Path, artifact_path: Path) -> None:
    evidence_path = root / "evidence/local/2026-07-27-v0.1.29/manifest.json"
...
def test_candidate_package_evidence_cannot_remain_pending(tmp_path: Path) -> None:
    ...
    assert "0.1.29 package evidence must record completed PASS verification" in failures
    assert "0.1.29 package evidence cannot retain pending command results" in failures
...
def test_release_verifier_requires_candidate_artifact_ignore_policy(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    (root / ".git").mkdir()
    (root / "evidence/local/2026-07-27-v0.1.29/artifacts/.gitignore").unlink(
        missing_ok=True
    )
...
def test_deployment_ledger_and_human_manifest_drift_fail_closed(tmp_path: Path) -> None:
    ...
    human.write_text(
        ...
        .replace(
            "SHA-256 `7be97ebc73c2ff70486aa60e019d7bbfa4bb05addb618ac338ab258c23f9b1a3`",
            "SHA-256 `" + "0" * 64 + "`",
            1,
        ),
        encoding="utf-8",
...
def test_artifact_self_hash_cannot_replace_archive_content_validation(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    wheel = root / "evidence/local/2026-07-27-v0.1.29/artifacts/agentnet-0.1.29-py3-none-any.whl"
...
def test_sdist_unsafe_or_extra_member_is_rejected(tmp_path: Path, mutation: str) -> None:
    root = _copy_release_inputs(tmp_path)
    sdist = root / "evidence/local/2026-07-27-v0.1.29/artifacts/agentnet-0.1.29.tar.gz"
    ...
    def mutate(entries: list[tuple[tarfile.TarInfo, bytes]]) -> None:
        ...
        member = tarfile.TarInfo(f"agentnet-0.1.29/{suffix}")
        member.size = 2
        member.mode = 0o644
