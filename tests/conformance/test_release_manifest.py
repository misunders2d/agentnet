...
def _copy_release_inputs(tmp_path: Path) -> Path:
    ...
    shutil.copytree(
        ROOT / "evidence/local/2026-08-09-v0.1.51/artifacts",
        root / "evidence/local/2026-08-09-v0.1.51/artifacts",
    )
    return root
...
def _refresh_artifact_hash(root: Path, artifact_path: Path) -> None:
    evidence_path = root / "evidence/local/2026-08-09-v0.1.51/manifest.json"
...
def test_candidate_evidence_must_cover_canonical_owner_recovery_surface(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    evidence_path = root / "evidence/local/2026-08-09-v0.1.51/manifest.json"
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    focused = next(
        ...
        if "tests/operations/test_canonical_owner_recovery.py" in record["command"]
    )
...
def test_release_verifier_requires_candidate_artifact_ignore_policy(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    (root / ".git").mkdir()
    (root / "evidence/local/2026-08-09-v0.1.51/artifacts/.gitignore").unlink(
        missing_ok=True
    )
...
def test_deployment_ledger_and_human_manifest_drift_fail_closed(tmp_path: Path) -> None:
    ...
    requirements_path.write_text(
        ...
        .replace("Snapshot: 2026-08-09.", "Snapshot: 2026-07-25.", 1)
        .replace(
    ...
    gate_path.write_text(
        gate_path.read_text(encoding="utf-8").replace(
            "Current ledger update: 2026-08-09.",
...
def test_artifact_self_hash_cannot_replace_archive_content_validation(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    wheel = root / "evidence/local/2026-08-09-v0.1.51/artifacts/agentnet-0.1.51-py3-none-any.whl"
...
def test_sdist_unsafe_or_extra_member_is_rejected(tmp_path: Path, mutation: str) -> None:
    root = _copy_release_inputs(tmp_path)
    sdist = root / "evidence/local/2026-08-09-v0.1.51/artifacts/agentnet-0.1.51.tar.gz"
