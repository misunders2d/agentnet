...
def _copy_release_inputs(tmp_path: Path) -> Path:
    ...
    for relative_path in (
        ...
        "evidence/local/2026-08-06-v0.1.45/manifest.json",
    ...
    shutil.copytree(
        ROOT / "evidence/local/2026-08-06-v0.1.45/artifacts",
        root / "evidence/local/2026-08-06-v0.1.45/artifacts",
    )
    return root
...
def _refresh_artifact_hash(root: Path, artifact_path: Path) -> None:
    evidence_path = root / "evidence/local/2026-08-06-v0.1.45/manifest.json"
...
def test_candidate_package_evidence_cannot_remain_pending(tmp_path: Path) -> None:
    ...
    assert "0.1.45 package evidence must record completed PASS verification" in failures
    assert "0.1.45 package evidence cannot retain pending command results" in failures


def test_candidate_evidence_must_cover_v0145_release_blocker_surface(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    evidence_path = root / "evidence/local/2026-08-06-v0.1.45/manifest.json"
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    focused = next(
        ...
        if "tests/integration/test_invitation_browser_journey.py" in record["command"]
    )
    ...
    assert "0.1.45 focused release-blocker evidence is incomplete" in failures
...
def test_sdist_contract_does_not_require_installed_root_ignore_files(tmp_path: Path) -> None:
    ...
    assert expected[".gitignore"].endswith(b".superpowers/\n")


def test_release_verifier_requires_candidate_artifact_ignore_policy(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    (root / ".git").mkdir()
    (root / "evidence/local/2026-08-06-v0.1.45/artifacts/.gitignore").unlink(
        missing_ok=True
    )
...
def test_deployment_ledger_and_human_manifest_drift_fail_closed(tmp_path: Path) -> None:
    ...
    human.write_text(
        ...
        .replace(
            "SHA-256 `56774c65eef4811b6f08ece2b760a9b0ed5a8a0641f92938a232ccbab930dd8c`",
            "SHA-256 `" + "0" * 64 + "`",
            1,
        ),
        encoding="utf-8",
    ...
    requirements_path.write_text(
        ...
        .replace("Snapshot: 2026-08-06.", "Snapshot: 2026-07-25.", 1)
        .replace(
    ...
    gate_path.write_text(
        gate_path.read_text(encoding="utf-8").replace(
                "Current ledger update: 2026-08-06.",
...
def test_artifact_self_hash_cannot_replace_archive_content_validation(tmp_path: Path) -> None:
    root = _copy_release_inputs(tmp_path)
    wheel = root / "evidence/local/2026-08-06-v0.1.45/artifacts/agentnet-0.1.45-py3-none-any.whl"
...
def test_sdist_unsafe_or_extra_member_is_rejected(tmp_path: Path, mutation: str) -> None:
    root = _copy_release_inputs(tmp_path)
    sdist = root / "evidence/local/2026-08-06-v0.1.45/artifacts/agentnet-0.1.45.tar.gz"
    ...
    def mutate(entries: list[tuple[tarfile.TarInfo, bytes]]) -> None:
        ...
        member = tarfile.TarInfo(f"agentnet-0.1.45/{suffix}")
        member.size = 2
        member.mode = 0o644
