...
def corporate_request(*, intent: str, grant: TaskGrant, suffix: str) -> SendMessageRequest:
...
async def test_corporate_message_and_task_use_exact_grants_mailbox_and_task_state_owners(
    ...
    direct_request = corporate_request(intent="message", grant=direct_grant, suffix="direct-0001")
    ...
    mailbox_rows = handler.mailbox.reconcile(recipient.harness_id)
    ...
    task_request = corporate_request(intent="task", grant=task_grant, suffix="task-0000001")
    ...
    assert len(handler.mailbox.reconcile(recipient.harness_id)) == 1
...
async def test_corporate_a2a_raw_and_unreleased_url_parts_fail_before_grant_or_mailbox_use(
    ...
    raw = corporate_request(intent="message", grant=raw_grant, suffix="raw-rejected-0001")
    ...
    unbound = corporate_request(intent="message", grant=url_grant, suffix="url-rejected-0001")
...
async def test_corporate_a2a_released_url_is_exactly_bound_and_non_fetchable_in_custody(
    ...
    request = corporate_request(intent="message", grant=grant, suffix="released-url-0001")
    ...
    item = handler.mailbox.reconcile(recipient.harness_id)[0]
