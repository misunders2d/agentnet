...
def _verify_built_artifacts(root: Path, artifacts: Any, failures: list[str]) -> None:
    base = "evidence/local/2026-08-06-v0.1.46/artifacts"
    ...
    if ignore_path.is_file():
        ...
        failures.append("final package artifact ignore policy does not retain its archives")
    expected_paths = {
        f"{base}/agentnet-0.1.46.tar.gz",
        f"{base}/agentnet-0.1.46-py3-none-any.whl",
    }
    ...
    try:
        with zipfile.ZipFile(wheel_path) as archive:
            ...
            dist_info = "agentnet-0.1.46.dist-info"
            shared = "agentnet-0.1.46.data/data/share/agentnet"
...
def _verify_evidence_ledgers(manifest: dict[str, Any], root: Path, failures: list[str]) -> None:
    ...
    package_evidence = _load_json(
        root / "evidence/local/2026-08-06-v0.1.46/manifest.json",
        failures,
        "0.1.46 package evidence manifest",
    )
    if package_evidence.get("release_source_tree_sha256") != _source_tree_sha256(root):
        failures.append("0.1.46 package evidence is not bound to the current source tree")
    if package_evidence.get("verification_status") != "PASS":
        failures.append("0.1.46 package evidence must record completed PASS verification")
    ...
    if not isinstance(command_records, list) or any(
        ...
        failures.append("0.1.46 package evidence commands are malformed")
    else:
        command_results = {
        ...
        if (
            ...
            or "2178 passed and 21 expected" not in npm_result
            ...
            failures.append("0.1.46 npm source and recursive packed evidence is incomplete")
        routing_result = next(
            (
                result
        ...
        if not all(marker in routing_result for marker in required_routing_markers):
            failures.append("0.1.46 packaged exact-endpoint routing evidence is incomplete")
        ...
        if not any(
            ...
            and result == "PASS: 543 passed, 5 expected dedicated-PostgreSQL skips"
            for command, result in command_results.items()
        ):
            failures.append("0.1.46 focused release-blocker evidence is incomplete")
        if not any(
            command.startswith("SOURCE_DATE_EPOCH=1580601600 ")
            and result.startswith("PASS: two independent builds")
            for command, result in command_results.items()
        ):
            failures.append("0.1.46 reproducible build evidence is incomplete")
        if any("PENDING" in result for result in command_results.values()):
            failures.append("0.1.46 package evidence cannot retain pending command results")
    ...
    if not isinstance(execution_context, dict) or not all(
        ...
        failures.append("0.1.46 package evidence execution context is incomplete")
