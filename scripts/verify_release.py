...
def _verify_built_artifacts(root: Path, artifacts: Any, failures: list[str]) -> None:
    base = "evidence/local/2026-07-14-v0.1.4/artifacts"
    ignore_path = root / base / ".gitignore"
    expected_ignore = "*\n!/.gitignore\n!/*.whl\n!/*.tar.gz\n"
    if not ignore_path.is_file() or ignore_path.read_text(encoding="utf-8") != expected_ignore:
        failures.append("final package artifact directory does not explicitly retain its archives")
    expected_paths = {
        f"{base}/agentnet-0.1.4.tar.gz",
        f"{base}/agentnet-0.1.4-py3-none-any.whl",
    }
    ...
    try:
        with zipfile.ZipFile(wheel_path) as archive:
            ...
            dist_info = "agentnet-0.1.4.dist-info"
            shared = "agentnet-0.1.4.data/data/share/agentnet"
...
def _verify_evidence_ledgers(manifest: dict[str, Any], root: Path, failures: list[str]) -> None:
    ...
    package_evidence = _load_json(
        root / "evidence/local/2026-07-14-v0.1.4/manifest.json",
        failures,
        "0.1.4 package evidence manifest",
    )
    if package_evidence.get("release_source_tree_sha256") != _source_tree_sha256(root):
        failures.append("0.1.4 package evidence is not bound to the current source tree")
