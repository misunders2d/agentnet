...
def _verify_built_artifacts(root: Path, artifacts: Any, failures: list[str]) -> None:
    base = "evidence/local/2026-07-13-final/artifacts"
    ...
    expected_paths = {
        f"{base}/agentnet-0.1.0.tar.gz",
        f"{base}/agentnet-0.1.0-py3-none-any.whl",
    }
    ...
    try:
        with zipfile.ZipFile(wheel_path) as archive:
            ...
            dist_info = "agentnet-0.1.0.dist-info"
            shared = "agentnet-0.1.0.data/data/share/agentnet"
...
def _verify_evidence_ledgers(manifest: dict[str, Any], root: Path, failures: list[str]) -> None:
    ...
    _verify_built_artifacts(root, local_evidence.get("artifacts", []), failures)
