...
EXPECTED_SOURCES = {
    "concept": (
        ...
        "a85fa244d8427106fe8272d9e92df99da34d36571d6b80e22edf71fcda3fa505",
    ),
    "requirements": (
...
def _verify_built_artifacts(root: Path, artifacts: Any, failures: list[str]) -> None:
    base = "evidence/local/2026-07-18-v0.1.13/artifacts"
    ...
    expected_paths = {
        f"{base}/agentnet-0.1.13.tar.gz",
        f"{base}/agentnet-0.1.13-py3-none-any.whl",
    }
    ...
    try:
        with zipfile.ZipFile(wheel_path) as archive:
            ...
            dist_info = "agentnet-0.1.13.dist-info"
            shared = "agentnet-0.1.13.data/data/share/agentnet"
...
def _verify_evidence_ledgers(manifest: dict[str, Any], root: Path, failures: list[str]) -> None:
    ...
    package_evidence = _load_json(
        root / "evidence/local/2026-07-18-v0.1.13/manifest.json",
        failures,
        "0.1.13 package evidence manifest",
    )
    if package_evidence.get("release_source_tree_sha256") != _source_tree_sha256(root):
        failures.append("0.1.13 package evidence is not bound to the current source tree")
