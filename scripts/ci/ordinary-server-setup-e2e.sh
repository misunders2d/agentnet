...
[[ "$REVISION_2" -eq "$REVISION_1" ]] || { echo "ordinary server setup E2E: stable retry changed revision" >&2; exit 1; }
[[ "$(sudo sha256sum /var/lib/agentnet-setup/setup.json | cut -d' ' -f1)" == "$MARKER_1" ]] || { echo "ordinary server setup E2E: stable retry changed marker bytes" >&2; exit 1; }
