...
Unreleased `0.1.32` keeps that protocol and adds one coherent blocker repair: signed Approval broker readiness through the configured public origin; authoritative setup response-loss reconciliation; recoverable OIDC-begin idempotency; finite 24-hour always-on credentials with selector-free six-hour-window renewal; Core schema v5; one package-owned isolated C0 responder that never resurrects after terminal cleanup; clean current-package setup-attempt custody; and a five-unit systemd lifecycle. One exact setup-owned `0.1.31 -> 0.1.32` transition accepts only the realized two-unit public marker and a retained, non-lab ordinary-server credential expired no more than 24 hours earlier. It proves possession of the retained key, preserves principal/harness/key and all authority/message state, advances the credential epoch once, records durable replay custody, installs the five-unit profile without starting services when apply was no-start, and grants no authority. It cannot chain a second replacement on the same harness or run from another package version. No production or gate-promotion claim is added. Required next runtime proof remains fresh-laptop enrollment, one native signed message, recipient `recipient_committed`, exact `COMPLETED_C0_ROUND_TRIP`, then five-power revocation and post-revocation refusal.
