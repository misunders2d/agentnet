...
Unreleased `0.1.32` is one coordinated repair for the ceremony blockers: signed public-path Approval broker readiness; authoritative setup reconciliation; OIDC-begin exact replay after response loss/concurrency; finite current-credential renewal; clean current-package setup-attempt custody; and a package-owned isolated C0 responder under a five-unit systemd lifecycle. Terminal responder state is owner-only, and same-digest setup repairs a marker-before-config-cleanup interruption without resurrecting the responder. The first-C0 path rejects reuse of `0.1.31` state. Local source, retained-artifact, recursive package, release-verifier, skill-loader, and code/skill review evidence passes; the two installed-harness G01 failures remain non-green and unwaived, and final Constitution evidence-only closure passes. Mutation-authorized PostgreSQL, cross-platform CI, and external fresh-host/C0 evidence remain absent; no 0.1.32 artifact is published, deployed, or certified.
