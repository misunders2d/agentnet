...
Before producing or following a live packet, read [the fresh-laptop onboarding contract](references/fresh-laptop-onboarding.md), resolve and verify every required placeholder in its canonical public onboarding prompt template from approved public metadata, and issue the resulting single-paste packet unchanged rather than splitting, handcrafting, shortening, or paraphrasing it. Any unresolved required placeholder blocks issuance. Verify the installed release actually ships every required step. The packet must cover the exact public installation source/version, OS/Node/`uv` prerequisites, install verification, join start, system-browser Google OIDC, independent approval, join completion, identity/connection verification, first test message, expected outputs, and safe failure guidance.
