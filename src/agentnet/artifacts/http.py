"""Authenticated artifact upload, release, download, and lifecycle HTTP routes."""
...
def create_artifact_routes(
    ...
    """Mount only artifact reservation, content, release, and lifecycle routes."""
    ...
    async def upload_artifact(request: Request) -> Response:
        ...
        if content_type == "application/octet-stream":
            ...
            content = decode_base64(parsed.content_b64, field="content_b64")
        else:
            raise ValidationError(
                "artifact upload requires application/octet-stream or application/json"
            )
    ...
    async def record_artifact_scan(request: Request) -> Response:
        ...
        return JSONResponse(
            core.artifacts.record_scan(artifact_id, parsed.attestation)
        )
    ...
    async def issue_download_capability(request: Request) -> Response:
        ...
        return JSONResponse(
            {"artifact_id": artifact_id, "capability": token}
        )
    ...
    async def artifact_lifecycle(request: Request) -> Response:
        ...
        core._require(
            actor=actor,
            action="artifact.lifecycle.read",
            resource=artifact_id,
        )
        return JSONResponse(
            core.artifacts.lifecycle_status(artifact_id, actor=actor)
        )
    ...
    return [
        ...
        Route(
            "/v1/artifacts/reservations/{reservation_id}/bytes",
            upload_artifact,
            methods=["POST"],
        ),
        Route(
            "/v1/artifacts/reservations/{reservation_id}/abort",
            abort_artifact_reservation,
            methods=["POST"],
        ),
        Route(
            "/v1/artifacts/reservations/{reservation_id}/promote",
            promote_artifact,
            methods=["POST"],
        ),
        Route(
            "/v1/artifacts/{artifact_id}/scan",
            record_artifact_scan,
            methods=["POST"],
        ),
        Route(
            "/v1/artifacts/{artifact_id}/release",
            release_artifact,
            methods=["POST"],
        ),
        Route(
            "/v1/artifacts/{artifact_id}/download-capabilities",
        ...
        Route(
            "/v1/artifacts/{artifact_id}/legal-hold",
            set_artifact_legal_hold,
            methods=["POST"],
        ),
        Route(
            "/v1/artifacts/{artifact_id}/legal-hold/clear",
            clear_artifact_legal_hold,
            methods=["POST"],
        ),
        Route(
...
__all__ = ["create_artifact_routes"]
